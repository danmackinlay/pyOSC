...
def parseUrlStr(url):
    """Convert provided string in 'host:port/prefix' format to its components
...
class OSCClient(object):
    """Simple OSC Client. Handles the sending of OSC-Packets
    (OSCMessage or OSCBundle) via a UDP-socket
    """
    ...
    def sendto(self, msg, address, timeout=None):
        """Send the given OSCMessage to the specified address.
          ...
          - timeout:  A timeout value for attempting to send. If timeout is None,
...
# A translation-table for mapping OSC-address expressions to Python
# 're' expressions
...
class OSCMultiClient(OSCClient):
    """
    'Multiple-Unicast' OSC Client. Handles the sending of OSC-Packets
    (OSCMessage or OSCBundle) via a UDP-socket
    This client keeps a dict of 'OSCTargets'
    and sends each OSCMessage to each OSCTarget.
    The OSCTargets are simply (host, port) tuples,
    and may be associated with an OSC-address prefix.
    the OSCTarget's prefix gets prepended
    to each OSCMessage sent to that target.
    """
    def __init__(self, server=None):
        """Construct a "Multi" OSC Client.
          - server: Local OSCServer-instance this client will use
          the socket of for transmissions.
          If none is supplied, a socket will be created.
        """
    ...
    def _searchHostAddr(self, host):
        """Search the subscribed OSCTargets for (the first occurence of)
        a given host.
    ...
    def _setTarget(self, address, prefix=None, filters=None):
        """
        Add (i.e. subscribe) a new OSCTarget,
        or change the prefix for an existing OSCTarget.
            - address ((host, port) tuple): IP-address & UDP-port
            - prefix (string): The OSC-address prefix prepended to
              the address of each OSCMessage
        ...
        if prefix is not None:
            if len(prefix):
                # make sure prefix starts with ONE '/',
        ...
        if filters is not None:
            if type(filters) in (str,):
                ...
                raise TypeError(
                    "'filters' argument must be a dict "
                    "with {addr:bool} entries")
    ...
    def setOSCTarget(self, address, prefix=None, filters=None):
        """Add (i.e. subscribe) a new OSCTarget,
        or change the prefix for an existing OSCTarget.
          the 'address' argument can be a ((host, port) tuple) :
            The target server address & UDP-port
            or a 'host' (string) : The host will be looked-up
          - prefix (string): The OSC-address prefix prepended
            to the address of each OSCMessage
            sent to this OSCTarget (optional)
        """
        if type(address) in (str,):
            ...
            raise TypeError(
                "'address' argument must be a (host, port) tuple"
                " or a 'host' string")

        self._setTarget(address, prefix, filters)

    def setOSCTargetFromStr(self, url):
        """Adds or modifies a subscribed OSCTarget from the given string,
    ...
    def _delTarget(self, address, prefix=None):
        ...
        If the 'prefix' argument is given, the Target is
        only deleted if the address and prefix match.
        """
        try:
            if prefix is None:
    ...
    def hasOSCTarget(self, address, prefix=None):
        ...
        the 'address' argument can be a ((host, port) tuple), or a hostname.
        If the 'prefix' argument is given, the return-value is only True
        if the address and prefix match.
        """
        if type(address) in (str,):
            address = self._searchHostAddr(address)
    ...
    def getOSCTarget(self, address):
        """Returns the OSCTarget matching the given address as
        a ((host, port), [prefix, filters]) tuple.
        'address' can be a (host, port) tuple, or a 'host' (string),
        in which case the first matching OSCTarget is returned
    ...
    def getOSCTargetStr(self, address):
        """Returns the OSCTarget matching the given address as a
        ('osc://<host>:<port>[<prefix>]', ['<filter-string>', ...])' tuple.
        'address' can be a (host, port) tuple, or a 'host' (string),
        in which case the first matching OSCTarget is returned
        Returns (None, []) if address not found.
        """
    ...
    def send(self, msg, timeout=None):
        """Send the given OSCMessage to all subscribed OSCTargets
          - msg:  OSCMessage (or OSCBundle) to be sent
          - timeout:  A timeout value for attempting to send.
              If timeout is None,
              this call blocks until socket is available for writing.
        Raises OSCClientError when timing out while waiting for the socket.
        """
...
class OSCRequestHandler(DatagramRequestHandler):
        ...
        for addr in list(self.server.callbacks.keys()):
            ...
            if match and (match.end() == len(addr)):
                ...
                if isinstance(reply, OSCMessage):
                    ...
                    raise TypeError(
                        "Message-callback %s did not return OSCMessage"
                        " or None: %s" %
    ...
    def finish(self):
        ...
        Send any reply returned by the callback(s) back
...
class OSCServer(UDPServer):
    ...
    If the adress-pattern of the message machtes the registered address
    of a callback,
    ...
    def setClient(self, client):
        """Associate this Server with a new local Client instance,
        closing the Client this Server is currently using.
        """
    ...
    def setReturnPort(self, port):
        """Set the destination UDP-port for replies returning from
        this server to the remote client
        """
    ...
    def setSrvInfoPrefix(self, pattern):
        """Set the first part of OSC-address (pattern) this server
        will use to reply to server-info requests.
        """
        if len(pattern):
...
if __name__ == "__main__":
    ...
    if addr is not None and addr[0] is not None:
